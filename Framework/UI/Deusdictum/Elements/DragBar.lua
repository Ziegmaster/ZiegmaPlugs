...
local deltaButtonAvailable = pcall( function( ) import (Framework.SelfPath .. ".UI.Deusdictum.Elements.DeltaButton"); end );

local res = string.gsub(Framework.SelfPath, "%.", "/") .. "/UI/Deusdictum/Resources";
...
Framework.UI.Controls.DragBar = class( Turbine.UI.Window );

function Framework.UI.Controls.DragBar:Constructor( ctlAttachTo, sName, bBarOnTop, tResources )
...
function Framework.UI.Controls.DragBar:CanHaveRotator( )
	if deltaButtonAvailable then
		return true;
	end
...
function Framework.UI.Controls.DragBar:SetRotatorOffset( oX, oY )
	if type( self.Rotator ) == "table" then
		self.Rotator:SetOffset( oX, oY );
	end
end

function Framework.UI.Controls.DragBar:SetRotator( bEnabled, tResources, bfZoneTest )
	if deltaButtonAvailable then
		if bEnabled then
			if type(tResources) ~= "table" then
				tResources = defaultRotatorResources;
			end
			self.Rotator = Framework.UI.Controls.DeltaButton( tResources, bfZoneTest, self );
...
function Framework.UI.Controls.DragBar:SetAllowsDragging( bValue )
...
function Framework.UI.Controls.DragBar:SetAllowsHUDHiding( bValue, bShowHUDIfHidden )
...
function Framework.UI.Controls.DragBar:GetAllowsDragging( )
	return self.bAllowDragging;
end

function Framework.UI.Controls.DragBar:GetAllowsHUDHiding( )
	return self.bAllowHUDHiding;
end

function Framework.UI.Controls.DragBar:HideTargetIfHUDHidden( )
	if self.wndTarget:IsVisible() and not self:IsHUDVisible() then
		self.wndTarget:SetVisible( false );
	end
end

function Framework.UI.Controls.DragBar:SetDraggable( bValue )
...
function Framework.UI.Controls.DragBar:IsDraggable( )
	return self.bReposUI;
end

function Framework.UI.Controls.DragBar:SetHUDVisible( bValue )
...
function Framework.UI.Controls.DragBar:IsHUDVisible( )
	return self.bHUDVisible;
end

function Framework.UI.Controls.DragBar:SetBeingDragged( bValue, args )
...
function Framework.UI.Controls.DragBar:IsBeingDragged( )
	return self.bDragging;
end

function Framework.UI.Controls.DragBar:ToggleDraggable( )
	self:SetDraggable( not self:IsDraggable() );
end

function Framework.UI.Controls.DragBar:ToggleHUDVisible( )
	self:SetHUDVisible( not self:IsHUDVisible() );
end

function Framework.UI.Controls.DragBar:CreateHoverBox( )
...
function Framework.UI.Controls.DragBar:DestroyHoverBox( )
	if self.hoverBox ~= nil then 
...
function Framework.UI.Controls.DragBar:IsBarOnTop( )
	return self.bOnTop;
end

function Framework.UI.Controls.DragBar:SetBarOnTop( bValue )
	self.bOnTop = bValue;

	self:RecalculatePosition();
end

function Framework.UI.Controls.DragBar:RecalculatePosition( )
	local X, Y;
...
function Framework.UI.Controls.DragBar:RecalculateSize()
	self:SetSize( self.wndTarget:GetWidth(), 19 );
	self.Bar:SetSize( self:GetWidth() - 19, 19 );
	self.Label:SetSize( self:GetWidth(), 18 );
...
function Framework.UI.Controls.DragBar:Refresh( )
