...
function ShortcutButton:Constructor()
	...
	self.enabledForeColor = controlColor;
...
function ShortcutButton:SyncControls()
	self:SetZOrder(1);
...
function ShortcutButton:SetText(msg)
	self.Button.Label:SetText(msg);
end

function ShortcutButton:SetShortcut(type, shortcut)
	self.chatSendShortcut = Turbine.UI.Lotro.Shortcut(type, shortcut);
	self.chatSend:SetShortcut(self.chatSendShortcut);
end

function ShortcutButton:Destroy()
	self.Background = nil;
